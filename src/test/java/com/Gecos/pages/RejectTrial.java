...
public class RejectTrial extends BasePageObject {
	RejectTrial strial = null;

	public RejectTrial(WebDriver driver) {
		super(driver);
	}
	...
	public static Logger logger = Logger.getLogger(RejectTrial.class);
...
public void clickCheckBox() throws Exception {
	String appName =getValFromExcel(8,4);
		...
		if(emailId.equals(email)){
			...
			for(WebElement orgToSerach : OrgId_Links){
		        ...
		        if(orgToSerach.getText().contains(Actual_org_text)&&app_ID.equals(appName)){ 
						...
						logger.info("The Trial is rejected for OrgID: "+Actual_org_text+" APP id "+appName);
...
public boolean isTrialDisplayedInManageTrials() throws Exception {
	try {
		...
		if (manageFlag) {
			...
			setElement(manageTrialLink).click();
			rejectTrial();
			logger.info("The Rejected trial is displayed in Manage trials page for the user");
		} else {
			logger.info("The trial is not rejected for the user");
		}
	} catch (Exception e) {
		throw new Exception("Home Link displayed" + isLoginPageDisplayed() + e.getLocalizedMessage());
	}
