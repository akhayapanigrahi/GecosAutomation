	...
	public boolean isdeleteLinkDisplayed() throws Exception{
		try {
			...
			if (deleteFlag) {
				...
				logger.info("Delete trials Page is accessible ");
				List<WebElement> delete_Links =driver.findElements(deleteLink);
				if(delete_Links.size()==0){
					logger.info("Delete Link is also Enabled");
				}
				else{
					logger.info("This user is not having Admin access and only Home page access is accessible");					
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
...
public boolean isRemoveButtonEnabled() throws Exception{
