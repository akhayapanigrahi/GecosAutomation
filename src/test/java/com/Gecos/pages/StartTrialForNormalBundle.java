...
public class StartTrialForNormalBundle extends BasePageObject {
	StartTrialForNormalBundle strial = null;

	public StartTrialForNormalBundle(WebDriver driver) {
		super(driver);
	}
	...
	public static Logger logger = Logger.getLogger(StartTrialForNormalBundle.class);
	...
	By app_id_text=By.xpath("//td[contains(text(),'a3x16000000Tj3r')]");
...
public void clickCheckBox() throws Exception {
	String appName =getValFromExcel(2,4);
...
public void matchEmailWithOrgID() throws Exception{
...
public boolean isTrialDisplayedInManageTrials() throws Exception {
	try {
		...
		if (manageFlag) {
			...
			matchEmailWithOrgID();
			logger.info("The started trial is displayed in Manage trials page for the user");
		} else {
			logger.info("The started trial is not started successfully for the user");
		}
	} catch (Exception e) {
