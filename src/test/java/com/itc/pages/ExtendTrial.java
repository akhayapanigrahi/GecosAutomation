...
public class ExtendTrial extends BasePageObject {
	ExtendTrial strial = null;

	public ExtendTrial(WebDriver driver) {
		super(driver);
	}
	...
	public static Logger logger = Logger.getLogger(ExtendTrial.class);
...
public void extendTrial() throws Exception{
			...
			for(WebElement orgToSerach : OrgId_Links){
		        ...
		        if(orgToSerach.getText().contains(Actual_org_text)&&app_ID.equals(appName)){ 
		        	...
		        	if(Trial_Status.equals("TRIAL STARTED")){
		        		By endDate=By.xpath("//td[@id='enddate"+Actual_org_text+appName+"']");
						String endDate_text_before = setElement(endDate).getText();
						logger.info("The End date Before Extended  is "+endDate_text_before);
						By ExtendTrialLink=By.xpath("//a[contains(@onclick,\"extendtrial('"+Actual_org_text+"','"+appName+"',0)\")]");
						//String Extend_Trial=setElement(ExtendTrialLink).getText();
						setElement(ExtendTrialLink).click();
						waitImplicit(3000);
		        ...
		        break;
		        	...
		        	else if(Trial_Status.equals("TRIAL STOPPED")){
		        		By ExtendTrialLink=By.xpath("//a[contains(@onclick,\"extendtrial('"+Actual_org_text+"','"+appName+"',0)\")]");
						setElement(ExtendTrialLink).click();
						waitImplicit(3000);
						Alert al=driver.switchTo().alert();
...
public boolean isTrialDisplayedInManageTrials() throws Exception {
	try {
		...
		if (manageFlag) {
			...
			extendTrial();
			logger.info("The Extended trial is displayed in Manage trials page for the user");
		} else {
			logger.info("The trial is not extended for the user");
		}
	} catch (Exception e) {
		throw new Exception("Home Link displayed" + isLoginPageDisplayed() + e.getLocalizedMessage());
	}
