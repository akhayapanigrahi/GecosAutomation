...
public class ApproveTrial extends BasePageObject {
	ApproveTrial strial = null;

	public ApproveTrial(WebDriver driver) {
		super(driver);
	}
	...
	public static Logger logger = Logger.getLogger(ApproveTrial.class);
...
public void clickCheckBox() throws Exception {
	String appName =getValFromExcel(6,4);
...
public void approveTrial() throws Exception{
		...
		if(emailId.equals(email)){
			...
			for(WebElement orgToSerach : OrgId_Links){
		        ...
		        if(orgToSerach.getText().contains(Actual_org_text)&&app_ID.equals(appName)){ 
						...
						By ApproveLink=By.xpath("//a[contains(@onclick,\"approvetrialapp('"+Actual_org_text+"','"+appName+"'\")]");
						setElement(ApproveLink).click();
						waitImplicit(3000);
						
						logger.info("The Trial is approved for OrgID: "+Actual_org_text+" APP id "+appName);
						
						
		        }
		       waitExplicit(manageTrialLink, 60);
			   setElement(manageTrialLink).click(); 		
			   By TrialStatusNext=By.xpath("//td[@id='status"+Actual_org_text+appName+"']");
			   String TrialStatutext = setElement(TrialStatusNext).getText();
			   if(TrialStatutext.equals("TRIAL STARTED")){
					waitExplicit(TrialStatusNext, 60);
			        	logger.info("The started trial is displayed in the Manage trials page for Org ID "+orgToSerach.getText()+" and App ID "+app_ID);
				   
			   }
			   else{
				   if(TrialStatutext.equals("PENDING APPROVAL")){
			        	logger.info("The started trial status is not changed the Manage trials page for Org ID "+orgToSerach.getText()+" and App ID "+app_ID);
...
public boolean isTrialDisplayedInManageTrials() throws Exception {
	try {
		...
		if (manageFlag) {
			...
			setElement(manageTrialLink).click();
			approveTrial();
			logger.info("The Approved trial is displayed in Manage trials page for the user");
		} else {
			logger.info("The trial is not Approved for the user");
		}
	} catch (Exception e) {
		throw new Exception("Home Link displayed" + isLoginPageDisplayed() + e.getLocalizedMessage());
	}
